...
def test_login_flow_contract_blinds_auth_popup_keys_by_context() -> None:
    auth_panel = _read(ROOT / 'ui' / 'auth_panel.py')
    access_gates = _read(ROOT / 'ui' / 'access_gates.py')
    app_shell = _read(ROOT / 'ui' / 'app_shell.py')
    auth_keys = _read(ROOT / 'ui' / 'auth_login_keys.py')

    required = [
        'build_auth_popup_key(',
        'context=context',
    ...
    haystack = auth_panel + '\n' + access_gates + '\n' + app_shell + '\n' + auth_keys
    for item in required:
        assert item in haystack, f'Blindagem de keys/contexto do login perdeu a âncora: {item}'
