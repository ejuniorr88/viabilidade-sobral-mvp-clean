...
def test_item_03_keeps_minimum_content_contract() -> None:
    assert_item_has_required_phrases(
        ...
        "estas siglas ajudam a interpretar corretamente"
        ],
    )


def test_item_03_content_does_not_fall_back_to_ui_relatorio() -> None:
    assert_relatorio_does_not_keep_residual_text(['| **A** | Adequado / permitido |'])
