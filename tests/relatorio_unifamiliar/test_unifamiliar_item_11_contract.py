...
def test_item_11_keeps_minimum_content_contract() -> None:
    assert_item_has_required_phrases(
        "item_11",
        [
            "Além das regras do lote",
            "ctx['render_quadro_tecnico']()",
        ],
    )


def test_item_11_content_does_not_fall_back_to_ui_relatorio() -> None:
    assert_relatorio_does_not_keep_residual_text(['Além das regras do lote'])
