...
def list_coupon_usage_rows(
    *,
    limit: int = 200,
    coupon_code_filter: Optional[str] = None,
    owner_email_filter: Optional[str] = None,
    ...
    coupon_rows = list_coupon_codes(limit=1000)
    coupons_by_id = {str(row.get("id")): row for row in coupon_rows}
    coupons_by_code = {_normalize_coupon_code(row.get("code")): row for row in coupon_rows}

    normalized_code = _normalize_coupon_code(coupon_code_filter) if coupon_code_filter else ""
    normalized_owner = _normalize_email(owner_email_filter) if owner_email_filter else ""
    normalized_status = str(payment_status_filter or "").strip().lower()

    enriched: List[Dict[str, Any]] = []
    for row in usage_rows:
        coupon_code = _normalize_coupon_code(row.get("coupon_code"))
        coupon_row = coupons_by_id.get(str(row.get("coupon_id"))) or coupons_by_code.get(coupon_code) or {}
        owner_email = _normalize_email(coupon_row.get("owner_email")) or None
        payment_status = str(row.get("payment_status") or "").strip().lower()

        merged = dict(row)
        merged["coupon_code"] = coupon_code or row.get("coupon_code")
        merged["owner_email"] = owner_email
        merged["payment_status"] = payment_status

        if normalized_code and merged.get("coupon_code") != normalized_code:
            continue
        if normalized_owner and (owner_email or "") != normalized_owner:
            continue
        if normalized_status and payment_status != normalized_status:
            continue
...
def summarize_coupon_usage_rows(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    total_usos = len(rows)
    usos_pagos = sum(1 for row in rows if str(row.get("payment_status") or "").strip().lower() == "paid")
    desconto_total = round(sum(_to_float(row.get("discount_amount"), 0.0) for row in rows), 2)
    valor_final_total = round(sum(_to_float(row.get("final_amount"), 0.0) for row in rows), 2)
    return {
        "total_usos": total_usos,
        "usos_pagos": usos_pagos,
