...
def render(ctx):
    ...
    if not ctx["rule"]:
        ...
        common.st.markdown(
            f"- **Taxa de Ocupação (TO) máxima:** {common._fmt_pct(ctx['to_max_pct'])}\n"
            f"- **Taxa de Permeabilidade (TP) mínima:** {common._fmt_pct(ctx['tp_min_pct'])}\n"
            f"- **Índice de Aproveitamento (IA) máximo:** {common._fmt_num(ctx['ia_max'], 2) if ctx['ia_max'] not in (None, '') else '—'}\n"
            f"- **Índice de Aproveitamento (IA) mínimo:** {common._fmt_num(ctx['ia_min'], 2) if ctx['ia_min'] not in (None, '') else 'não informado'}\n"
