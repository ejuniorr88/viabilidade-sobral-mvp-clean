...
def render(ctx):
    ...
    if ctx.get("built_ground") is not None and ctx.get("a_adotada") is not None:
        if ctx.get("excedeu_area"):
            common.st.markdown(
                f"👉 **Área pretendida informada:** **{common._fmt_num(ctx['built_ground'])} m²**. Como esse valor ultrapassa o limite adotado no relatório, os cálculos abaixo passam a considerar **{common._fmt_num(ctx['a_adotada'])} m²** no térreo."
            )
        else:
            common.st.markdown(
                f"👉 **Área pretendida informada:** **{common._fmt_num(ctx['built_ground'])} m²**. Como esse valor cabe dentro dos limites urbanísticos do lote, os cálculos abaixo passam a considerar essa mesma área no térreo."
            )
    ...
    common.st.markdown(
        f"👉 Nesse caso, o aproveitamento do térreo pode chegar ao limite máximo de **{common._fmt_num(ctx['to_m2'])} m²** pela Taxa de Ocupação (TO), desde que também sejam respeitadas as demais exigências urbanísticas aplicáveis ao caso."
    )
    ...
    if ctx.get("built_ground") is not None and ctx.get("a_adotada") is not None:
        if ctx.get("excedeu_area"):
            leitura_area = (
                f"Como a área pretendida informada foi de **{common._fmt_num(ctx['built_ground'])} m²**, mas o relatório precisou adotar **{common._fmt_num(ctx['a_adotada'])} m²** para respeitar os limites urbanísticos do lote, essa é a área considerada nesta leitura. "
            )
        else:
            leitura_area = (
                f"Como a área pretendida informada foi de **{common._fmt_num(ctx['built_ground'])} m²** e cabe dentro dos limites urbanísticos do lote, essa própria área foi considerada nesta leitura. "
            )
    common.st.markdown(
