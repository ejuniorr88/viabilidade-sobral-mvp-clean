...
# Helpers adicionados para blindagem dos relatórios ZEIP/R2.1/envelope físico
def zone_key(value: Any) -> str:
    return str(value or "").strip().upper().replace("-", "_").replace(" ", "_")

def is_zeip(ctx: Dict[str, Any]) -> bool:
    return zone_key(ctx.get("zone_sigla") or ctx.get("zone") or ctx.get("zone_title") or "").startswith("ZEIP")

def is_zeip9(ctx: Dict[str, Any]) -> bool:
    vals = [ctx.get("zone_sigla"), ctx.get("zone"), ctx.get("subzone_code"), ctx.get("zone_title")]
    keys = {zone_key(v) for v in vals if v is not None}
...
def is_r21_ctx(ctx: Dict[str, Any]) -> bool:
...
def is_r3_ctx(ctx: Dict[str, Any]) -> bool:
    multi_tipo = str(ctx.get("multi_tipo") or "").upper()
    use_type_code = str(ctx.get("use_type_code") or "").upper()
    return multi_tipo in ("R3", "R03") or use_type_code.endswith("R3")

def practical_ground_limit(ctx: Dict[str, Any]) -> Optional[float]:
    vals: List[float] = []
    for key in ("to_m2", "A_recuos"):
        v = _num(ctx.get(key))
        if v is not None and v > 0:
            vals.append(v)
    lot = _num(ctx.get("lot_area_f")); tp = _num(ctx.get("tp_m2"))
    if lot is not None and tp is not None:
        vals.append(max(lot - tp, 0.0))
    return min(vals) if vals else None

def dimension_alerts(ctx: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    rule = ctx.get("rule") or {}
    area = _num(ctx.get("lot_area_f")); front = _num(ctx.get("frontage_f") or ctx.get("W"))
