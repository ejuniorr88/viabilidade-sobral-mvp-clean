...
def render(ctx: dict) -> None:
    st = ctx["st"]
    md("**Para o uso residencial multifamiliar, a permissão pode depender principalmente da zona e, em alguns casos, também do tipo da via e do porte do empreendimento.**")
    if not ctx['zone_class'] and not ctx['via_class']:
        ...
        via_line = f"- **Por via:** {ctx['via_class']} ({ctx['_sigla_nome'](ctx['via_class'])})" if ctx['via_norm'] and ctx['via_class'] else f"- **Por via:** {ctx['via_tipo_txt'] or 'via local'}"
        md(
            f"- **Por zona:** {ctx['zone_class'] or 'não encontrado'}"
            + (f" ({ctx['_sigla_nome'](ctx['zone_class'])})" if ctx['zone_class'] else "")
            + "\n"
            + via_line
