...
def render(ctx: dict) -> None:
    ...
    if area_pedida is not None and area_considerada is not None:
        if excedeu_area:
            md("<!-- ultrapassa o limite máximo permitido pela TO; TO correspondente à área pretendida -->")
            md(
            ...
            if a_recuos is not None and float(area_pedida) <= float(a_recuos):
                ...
                md(f"<!-- cabe fisicamente pelos recuos, mas não pode ser adotada porque ultrapassa a Taxa de Ocupação máxima; limite real continua sendo {fmt_num(area_considerada)} m² no térreo; projeto precisaria ser reduzido para respeitar no máximo {fmt_num(area_considerada)} m² no térreo -->")
        else:
            md("<!-- TO correspondente à área pretendida -->")
            md(
    ...
    if ctx.get("is_irregular"):
        ...
        if area_pedida is not None and area_considerada is not None:
            if excedeu_area:
                md("<!-- ultrapassa o limite máximo permitido pela TO -->")
