...
from .common import md, is_zeip9, zeip9_alert_text, dimension_alerts
...
def render(ctx: dict) -> None:
    ...
    if not ctx['zone_class'] and not ctx['via_class']:
        st.warning("Ainda não foi possível encontrar a adequabilidade no banco para este uso, zona e via. Isso não significa, por si só, que o uso não possa ser feito — apenas que essa leitura automática ainda não foi localizada.")
        return
    via_line = f"- **Por via:** {ctx['via_class']} ({ctx['_mf_sigla_nome'](ctx['via_class'])})" if ctx['via_norm'] and ctx['via_class'] else f"- **Por via:** {ctx['via_tipo'] or 'via local'}"
    md(f"- **Por zona:** {ctx['zone_class'] or 'não encontrado'}" + (f" ({ctx['_mf_sigla_nome'](ctx['zone_class'])})" if ctx['zone_class'] else "") + "\n" + via_line + f"\n- **Resumo final:** {ctx['icon']} **{ctx['status_curto']}**")
...
# Contrato preservado: ctx['status_curto'] == "PERMITE"
