...
def render_relatorio_section(calc: Dict[str, Any]) -> None:
    ...
    st.checkbox("Documento de identidade do requerente ou representante legal", value=False, disabled=True)
    st.checkbox("CPF ou CNPJ", value=False, disabled=True)
    st.checkbox("Matrícula atualizada do imóvel ou documento equivalente", value=False, disabled=True)
    st.checkbox("Certidão negativa de IPTU", value=False, disabled=True)
    st.checkbox("Parecer favorável de Adequabilidade Locacional", value=False, disabled=True)
    st.checkbox("Tabela com índices urbanísticos e áreas da edificação", value=False, disabled=True)
    st.checkbox("Projeto arquitetônico em arquivo digital", value=False, disabled=True)
    st.checkbox("ART/RRT do responsável técnico", value=False, disabled=True)
    st.checkbox("Termo de responsabilidade do responsável técnico", value=False, disabled=True)
    st.checkbox("Termo de responsabilidade do proprietário", value=False, disabled=True)
    st.checkbox("Isenção da licença ambiental", value=False, disabled=True)

    st.markdown("**📌 Atenção**")
    st.checkbox("Confirmar se o caso realmente se enquadra como simplificado", value=False, disabled=True)
    st.checkbox("Conferir se a área construída está dentro do limite permitido", value=False, disabled=True)
    st.checkbox("Protocolar o pedido com antecedência mínima indicada pelo procedimento", value=False, disabled=True)
    st.checkbox("Verificar se todos os arquivos digitais estão prontos e legíveis", value=False, disabled=True)
    ...
    st.checkbox("Requerimento único", value=False, disabled=True)
    st.checkbox("Documento de identidade do requerente ou representante legal", value=False, disabled=True, key="obra_nova_doc_id")
    st.checkbox("CPF ou CNPJ", value=False, disabled=True, key="obra_nova_cpf_cnpj")
    st.checkbox("Matrícula atualizada do imóvel", value=False, disabled=True)
    st.checkbox("Autorização do proprietário, quando necessária", value=False, disabled=True)
    st.checkbox("BCI", value=False, disabled=True)
    st.checkbox("ART/RRT com comprovante de pagamento", value=False, disabled=True)
    st.checkbox("Projeto arquitetônico assinado", value=False, disabled=True)
    st.checkbox("Projeto hidrossanitário", value=False, disabled=True)
    st.checkbox("Memorial de cálculo e drenagem pluvial", value=False, disabled=True)
    st.checkbox("Declaração do SAAE sobre rede de esgoto, quando necessária", value=False, disabled=True)

    st.markdown("**✅ Checklist — documentos adicionais que podem ser exigidos**")
    st.checkbox("Aprovação do Corpo de Bombeiros", value=False, disabled=True)
    st.checkbox("Aprovação do IPHAN, quando o imóvel estiver em ZEIP", value=False, disabled=True)
    st.checkbox("Licenciamento ambiental ou termo de isenção", value=False, disabled=True)
    st.checkbox("PGRSCC", value=False, disabled=True)
    st.checkbox("Autorização do COMAR, quando aplicável", value=False, disabled=True)
    st.checkbox("Aprovação do DNIT ou SOP, quando houver acesso por rodovia", value=False, disabled=True)
    st.checkbox("EIV, quando exigido pela legislação", value=False, disabled=True)

    st.markdown("**📌 Atenção**")
    st.checkbox("Confirmar se o caso realmente exige alvará regular de obra nova", value=False, disabled=True)
    st.checkbox("Conferir se há exigência de documentos complementares por localização ou tipologia", value=False, disabled=True)
    st.checkbox("Verificar se o imóvel está em área com proteção especial", value=False, disabled=True)
    st.checkbox("Conferir se o projeto atende às exigências técnicas antes do protocolo", value=False, disabled=True)
