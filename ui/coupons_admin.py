...
from core.coupons import create_coupon_code, list_coupon_codes, list_coupon_usages, set_coupon_active, summarize_coupon_usages, update_coupon_code, user_can_manage_coupons
...
def render_coupons_admin_section(*, current_user_email: str) -> None:
    ...
    with u1:
        coupon_filter = st.text_input("Filtrar por código do cupom")
    with u2:
        owner_filter = st.text_input("Filtrar por e-mail do dono")
    with u3:
        status_filter = st.selectbox("Filtrar por status", options=["", "paid", "pending", "failed", "cancelled", "refunded"], index=0)

    usage_rows = list_coupon_usages(
        limit=200,
        coupon_code=coupon_filter or None,
        owner_email=owner_filter or None,
        payment_status=status_filter or None,
    )

    summary = summarize_coupon_usages(usage_rows)
    s1, s2, s3, s4 = st.columns(4)
    with s1:
        st.metric("Total de usos", int(summary.get("total_usages") or 0))
    with s2:
        st.metric("Usos pagos", int(summary.get("paid_usages") or 0))
    with s3:
        st.metric("Desconto total", f"R$ {float(summary.get('discount_total') or 0):,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
    with s4:
        st.metric("Valor final total", f"R$ {float(summary.get('final_amount_total') or 0):,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
    ...
    if usage_rows:
        ...
        st.info("Nenhum uso confirmado encontrado com os filtros atuais.")
