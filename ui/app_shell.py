...
def _nav_href(app_url: str, nav: str) -> str:
    ...
    return f"{base}/?vf_nav={escape(nav, quote=True)}" if base else f"/?vf_nav={escape(nav, quote=True)}"
...
def _build_top_nav_html(app_url: str) -> str:
    ...
    for label, key in links:
        href = escape(_nav_href(app_url, key), quote=True)
        nav_parts.append(
            f'<a class="vf-header-link" href="{href}" '
            f'onclick="event.stopPropagation(); window.location.href=this.href; return false;">{escape(label)}</a>'
        )
    nav_html = "".join(nav_parts)
    home_href = escape((app_url or "").rstrip("/") + "/?", quote=True) if app_url else "/?"
    return (
        '<div class="vf-topbar-shell">'
        '  <div class="vf-topbar-inner">'
        f'    <a class="vf-brand" href="{home_href}" onclick="event.stopPropagation(); window.location.href=this.href; return false;">Viabilidade-Fácil<span class="vf-brand-dot">.</span></a>'
