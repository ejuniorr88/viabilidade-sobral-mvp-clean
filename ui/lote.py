...
def render_lote_section() -> Tuple[float, float, float]:
    ...
    - Se "Terreno irregular" estiver MARCADO:
        ...
        (avisa que não é área do térreo e que recuos não serão calculados)
    - "Área pretendida no térreo": se 0, o relatório assume o máximo permitido pela zona.
    - Área permeável prevista é uma base: area_lote - area_terreo.
    """
    ...
    lot_area_used = float(testada or 0.0) * float(profund or 0.0)
    if is_irregular:
        st.info(
        ...
        lot_area_used = st.number_input(
            "Área do lote (m²)",
            min_value=0.0,
            value=float(st.session_state.get("lot_area_override_m2") or lot_area_used or 0.0),
            step=10.0,
            format="%.2f",
    ...
    st.checkbox(
        "Lote de esquina",
        value=bool(st.session_state.get("lot_is_corner") or False),
        key="lot_is_corner",
    )
